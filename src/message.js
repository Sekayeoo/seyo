...
async function GroupUpdate(seyo, update, store) {
	try {
		for (let n of update) {
...
async function GroupParticipantsUpdate(seyo, { id, participants, action }, store) {
	try {
		if (global.db.groups && global.db.groups[id] && global.db.groups[id].welcome && store.groupMetadata && store.groupMetadata[id]) {
			...
			for (let n of participants) {
				let profile;
				try {
					profile = await seyo.profilePictureUrl(n, 'image');
				} catch {
					profile = 'https://telegra.ph/file/95670d63378f7f4210f03.png';
				}
				if (action == 'add') {
					await seyo.sendMessage(id, {
...
async function LoadDataBase(seyo, m) {
	try {
...
async function MessagesUpsert(seyo, message, store) {
	try {
		let botNumber = await seyo.decodeJid(seyo.user.id);
		const msg = message.messages[0];
		if (store.groupMetadata && Object.keys(store.groupMetadata).length === 0) store.groupMetadata = await seyo.groupFetchAllParticipating()
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		if (!seyo.public && !msg.key.fromMe && message.type === 'notify') return
		if (!msg.message) return
		const m = await Serialize(seyo, msg, store)
		require('../seyo')(seyo, m, message, store);
		if (type === 'interactiveResponseMessage' && m.quoted && m.quoted.fromMe) {
			let apb = await generateWAMessage(m.chat, { text: JSON.parse(m.msg.nativeFlowResponseMessage.paramsJson).id, mentions: m.mentionedJid }, {
				userJid: seyo.user.id,
				quoted: m.quoted
			});
			apb.key = msg.key
			apb.key.fromMe = areJidsSameUser(m.sender, seyo.user.id);
			...
			seyo.ev.emit('messages.upsert', pbr);
		}
		if (global.db.set && global.db.set[botNumber] && global.db.set[botNumber].readsw) {
			if (msg.key.remoteJid === 'status@broadcast') {
				await seyo.readMessages([msg.key]);
				if (/protocolMessage/i.test(type)) seyo.sendFromOwner(global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
				if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
					...
					await seyo.sendFromOwner(global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
				}
			}
		}
	} catch (e) {
...
async function Solving(seyo, store) {
	seyo.public = true
	
	seyo.serializeM = (m) => MessagesUpsert(seyo, m, store)
	...
	seyo.getName = (jid, withoutContact  = false) => {
		const id = seyo.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || seyo.groupMetadata(id) || {};
	...
	seyo.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		let list = []
		for (let i of kon) {
			list.push({
				displayName: await seyo.getName(i + '@s.whatsapp.net'),
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await seyo.getName(i + '@s.whatsapp.net')}\nFN:${await seyo.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD` //vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await seyo.getName(i + '@s.whatsapp.net')}\nFN:${await seyo.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:whatsapp@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/seyo_dev\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
			})
		}
		seyo.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
	}
	
	seyo.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
		const result = await seyo.query({
			tag: 'iq',
			attrs: {
	...
	seyo.sendPoll = (jid, name = '', values = [], selectableCount = 1) => {
		return seyo.sendMessage(jid, { poll: { name, values, selectableCount }})
	}
	
	seyo.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return seyo.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, { quoted });
			} else if (mime && mime === 'application/pdf') {
				return seyo.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, { quoted });
			} else if (mime && mime.includes('image')) {
				return seyo.sendMessage(jid, { image: res.data, caption: caption, ...options }, { quoted });
			} else if (mime && mime.includes('video')) {
				return seyo.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, { quoted });
			} else if (mime && mime.includes('audio')) {
				return seyo.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, { quoted });
			}
		}
	...
	seyo.sendFakeLink = async (jid, text, title, body, thumbnail, myweb, options = {}) => {
		await seyo.sendMessage(jid, {
			text: text,
			contextInfo: {
				externalAdReply: {
					title: title,
	...
	seyo.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await seyo.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted });
		}
	}
	
	seyo.sendTextMentions = async (jid, text, quoted, options = {}) => seyo.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
	
	seyo.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		await seyo.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted });
		return buff;
	}
	
	seyo.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
	...
	seyo.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await seyo.getFile(path, true);
		...
		await seyo.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options });
		return fs.promises.unlink(pathFile);
	}
	
	seyo.sendButtonMsg = async (jid, body = '', footer = '', title = '', media, buttons = [], quoted, options = {}) => {
		...
		const hasil = await seyo.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
		return hasil
	}
	
	seyo.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
		async function getImageMsg(url) {
			const { imageMessage } = await generateWAMessageContent({ image: { url } }, { upload: seyo.waUploadToServer });
			return imageMessage;
		}
...
async function Serialize(seyo, m, store) {
	const botNumber = seyo.decodeJid(seyo.user.id)
	if (!m) return m
	if (m.key) {
		m.id = m.key.id
		...
		m.sender = seyo.decodeJid(m.fromMe && seyo.user.id || m.participant || m.key.participant || m.chat || '')
		if (m.isGroup) {
			m.metadata = store.groupMetadata[m.chat] || await seyo.groupMetadata(m.chat)
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.sender = seyo.decodeJid(m.msg.contextInfo.participant)
			m.quoted.fromMe = m.quoted.sender === seyo.decodeJid(seyo.user.id)
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, seyo)
				return await Serialize(seyo, q, store)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(seyo.decodeJid(m.msg?.contextInfo?.participant), seyo.decodeJid(seyo?.user?.id)),
			...
			m.quoted.delete = () => {
				seyo.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.copy = () => Serialize(seyo, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
		...
		try {
			if (/^https?:\/\//.test(text)) {
				...
				if (/gif|image|video|audio|pdf/i.test(mime)) {
					return seyo.sendFileUrl(chatId, text, caption, quoted, options)
				} else {
					return seyo.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
				}
			} else {
				return seyo.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
			}
